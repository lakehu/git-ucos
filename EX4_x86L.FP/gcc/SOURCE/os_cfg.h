...
#define OS_TASK_DEF_STK_SIZE   2000    /* Default stack size (# of OS_STK wide entries).  NOTE: the    */
                                       /* Linux/POSIX port runs real host code (scheduler, sprintf,   */
                                       /* sigprocmask) on each task stack, so the tiny 16-bit DOS      */
                                       /* sizes are too small and overflow -> SIGSEGV.                 */

#define OS_TASK_IDLE_STK_SIZE   OS_TASK_DEF_STK_SIZE   /* Idle task stack size (# of OS_STK entries)   */

#define OS_TASK_STAT_EN           1    /* Enable (1) or Disable(0) the statistics task                 */
#define OS_TASK_STAT_STK_SIZE   OS_TASK_DEF_STK_SIZE   /* Statistics task stack size (# of OS_STK)     */
...
#define OS_TICKS_PER_SEC        200    /* Set the number of ticks in one second                        */
                                       ...
                                       /* ------------- LINUX/POSIX PORT ADDITIONS ------------------ */
                                       /* (defines required by the ported uC/OS-II kernel that were   */
                                       /*  not present in the original Borland DOS os_cfg.h)          */
#define OS_TASK_SW_HOOK_EN        1    /*     Include code for OSTaskSwHook()                          */
#define OS_TASK_STAT_HOOK_EN      1    /*     Include code for OSTaskStatHook()                        */
#define OS_TIME_TICK_HOOK_EN      1    /*     Include code for OSTimeTickHook()                        */
#define OS_TASK_STAT_STK_CHK_EN   1    /*     Check task stacks from the statistics task              */
...
#define OS_TASK_NAME_SIZE        32    /*     Size of a task name                                      */
#define OS_EVENT_NAME_SIZE       32    /*     Size of the name of a Sem, Mutex, Mbox or Q             */
#define OS_FLAG_NAME_SIZE        32    /*     Size of the name of an event flag group                 */
#define OS_MEM_NAME_SIZE         32    /*     Size of a memory partition name                          */
